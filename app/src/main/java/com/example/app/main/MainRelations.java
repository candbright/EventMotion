...
public class MainRelations extends BaseExternalRelations<MainActivity> {
    ...
    public MainRelations(MainActivity activity) {
        ...
        navigationBottomBarManager = new NavigationBottomBarManager(activity.navigationBarBottom);
        navigationBottomBarManager.setMusicButtonClickListener(v -> {

        });
        navigationBottomBarManager.setSquareButtonClickListener(v -> {
        ...
        activity.getRootBinding().refreshLayout.setOnRefreshListener(() -> {
            List<Song> songsSelect = songDaoHelper.searchByModeAndDetail(activity.mode, activity.modeDetail);
            activity.setData(songsSelect);
            activity.getRootBinding().refreshLayout.setRefreshing(false);
        });
        activity.selectorBar.allModeBtn.setOnClickListener(v -> {
            List<Song> songsSelect = songDaoHelper.searchByModeAndDetail(MODE_ALL, activity.modeDetail);
            activity.setData(songsSelect);
            activity.mode = MODE_ALL;
            activity.matchMode();
        });
        activity.selectorBar.fancyModeBtn.setOnClickListener(v -> {
            List<Song> songsSelect = songDaoHelper.searchByModeAndDetail(MODE_FANCY, activity.modeDetail);
            activity.setData(songsSelect);
            activity.mode = MODE_FANCY;
            activity.matchMode();
        });
        activity.selectorBar.raceModeBtn.setOnClickListener(v -> {
            List<Song> songsSelect = songDaoHelper.searchByModeAndDetail(MODE_RACE, activity.modeDetail);
            activity.setData(songsSelect);
            activity.mode = MODE_RACE;
            activity.matchMode();
        });
        activity.selectorBar.allDetailBtn.setOnClickListener(v -> {
            List<Song> songsSelect = songDaoHelper.searchByModeAndDetail(activity.mode, MODE_DETAIL_ALL);
            activity.setData(songsSelect);
            activity.modeDetail = MODE_DETAIL_ALL;
            activity.matchModeDetail();
        });
        activity.selectorBar.studyDetailBtn.setOnClickListener(v -> {
            List<Song> songsSelect = songDaoHelper.searchByModeAndDetail(activity.mode, STUDY);
            activity.setData(songsSelect);
            activity.modeDetail = STUDY;
            activity.matchModeDetail();
        });
        activity.selectorBar.performDetailBtn.setOnClickListener(v -> {
            List<Song> songsSelect = songDaoHelper.searchByModeAndDetail(activity.mode, PERFORM);
            activity.setData(songsSelect);
            activity.modeDetail = PERFORM;
            activity.matchModeDetail();
        });
        activity.selectorBar.insaneDetailBtn.setOnClickListener(v -> {
            List<Song> songsSelect = songDaoHelper.searchByModeAndDetail(activity.mode, INSANE);
            activity.setData(songsSelect);
            activity.modeDetail = INSANE;
            activity.matchModeDetail();
        });
        activity.selectorBar.doubleDetailBtn.setOnClickListener(v -> {
            List<Song> songsSelect = songDaoHelper.searchByModeAndDetail(activity.mode, DOUBLE);
            activity.setData(songsSelect);
            activity.modeDetail = DOUBLE;
            activity.matchModeDetail();
        });
        activity.selectorBar.loverDetailBtn.setOnClickListener(v -> {
            List<Song> songsSelect = songDaoHelper.searchByModeAndDetail(activity.mode, LOVER);
            activity.setData(songsSelect);
            activity.modeDetail = LOVER;
            activity.matchModeDetail();
        });
        activity.selectorBar.remixDetailBtn.setOnClickListener(v -> {
            List<Song> songsSelect = songDaoHelper.searchByModeAndDetail(activity.mode, REMIX);
            activity.setData(songsSelect);

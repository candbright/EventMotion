...
public abstract class BaseDiffAdapter extends RecyclerView.Adapter<BaseViewHolder> {
    ...
    protected final List<SortedItem> datas; // 数据源

    public BaseDiffAdapter(List<SortedItem> datas) {
        this.datas = datas;
    ...
    public void notifyDiff() {
        DiffUtil.DiffResult diffResult = DiffUtil.calculateDiff(new DiffUtil.Callback() {
            ...
            public boolean areItemsTheSame(int oldItemPosition, int newItemPosition) {
                return BaseDiffAdapter.this.areItemsTheSame(temp.get(oldItemPosition), datas.get(newItemPosition));
            }

            // 如果是同一个 item 判断内容是否相同
            @Override
            public boolean areContentsTheSame(int oldItemPosition, int newItemPosition) {
                return BaseDiffAdapter.this.areContentsTheSame(temp.get(oldItemPosition), datas.get(newItemPosition));
            }
        });
